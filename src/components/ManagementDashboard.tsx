      ...
      <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
        {activeTab === 'home' && <>
          ...
          <GlassCard className="space-y-3">
            ...
            {filteredStudents.map((s) => { const admit = admitCards.find((a) => a.student_id === s.id); return <div key={s.id} className="flex items-center justify-between rounded-xl bg-white/10 p-3"><div><p>{s.full_name}</p><p className="text-xs text-slate-300">Reg: {formsByStudent.get(s.id)?.registration_id || 'No Reg ID'}</p><p className="text-xs text-slate-400">Exam: {admit?.exam_date || 'N/A'} • {admit?.exam_centre || 'N/A'}</p></div><div className="flex gap-2">{formsByStudent.get(s.id)?.status !== 'approved' && <GlassButton size="sm" onClick={() => approveScholarship(s.id)}><CheckCircle2 size={14} />Approve</GlassButton>}</div></div>; })}
          </GlassCard>
