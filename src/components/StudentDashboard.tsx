...
export const StudentDashboard = ({ user, profile, onLogout }: { user: any, profile: any, onLogout: () => void }) => {
  ...
  const fetchData = async () => {
    const [notifyRes, scholarshipRes, admitRes, resultRes, classRes, testsRes, testPayRes, historyRes] = await Promise.all([
      ...
      supabase.from('scholarship_forms').select('id,student_id,registration_id,payment_status,status,fee_paid,created_at,data,exam_date,exam_centre').eq('student_id', user.id).order('created_at', { ascending: false }).limit(1).maybeSingle(),
    ...
    setScholarshipRecord(scholarshipRes.data ? { ...scholarshipRes.data, exam_date: admitRes.data?.exam_date || scholarshipRes.data.exam_date, exam_centre: admitRes.data?.exam_centre || scholarshipRes.data.exam_centre } : null);
  ...
  const applyAndPayScholarship = async () => {
    const registrationId = scholarshipRecord?.registration_id || generateRegistrationId();
      ...
      <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
        ...
        {activeTab === 'scholarships' && (
          <GlassCard className="space-y-4">
            ...
            <p>Exam Date: <strong>{admitCard?.exam_date || scholarshipRecord?.exam_date || 'Not published'}</strong> | Centre: <strong>{admitCard?.exam_centre || scholarshipRecord?.exam_centre || 'Not published'}</strong></p>
